...
SELECT country, COUNT(country) AS count_movies
FROM movie
...
SELECT genre, COUNT(*) AS count_movies
FROM genre
GROUP BY genre
...
SELECT 
    ...
    RANK() OVER(ORDER BY ROUND(SUM(avg_rating * r.total_votes) / SUM(r.total_votes), 2) DESC) AS actor_rank   
FROM 
    movie AS m  
INNER JOIN 
...
FROM (
    ...
    GROUP BY 
        genre, year, movie_name, worlwide_gross_income  -- Include worlwide_gross_income in GROUP BY clause
) AS movie_summary
WHERE 
    movie_rank <= 5
ORDER BY 
